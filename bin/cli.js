...
class LoggerService {
  ...
  getIndent(char = "├") {
    ...
    let indent = "";
    if (this.indent - 1 > 0) {
      indent += source_default.grey("│ ").repeat(this.indent - 1);
    }
    indent += source_default.grey(char) + " ";
    return indent;
  }
  group(icon, message) {
...
class PackageManagerService {
  ...
  getInstallCommand(packageName, isDev = false, exact = false) {
    ...
    const exactFlag = exact ? this.selectedPM === "npm" || this.selectedPM === "pnpm" ? "--save-exact " : "--exact " : "";
  ...
  async loadDefaultPackageManager() {
    if (!this.selectedPM) {
...
function isYamlFile(recipePath) {
...
program2.command("recipe").description("Execute a recipe").argument("<name-or-path>", "Recipe name / Path to local recipe file").action(async (recipeNameOrPath) => {
  try {
    ...
    if (options.pm) {
      ...
      packageManagerService.setPackageManager(options.pm);
    } else {
