  ...
  class Command extends EventEmitter4 {
    constructor(name2) {
      ...
      this._outputConfiguration = {
        ...
        getOutHelpWidth: () => process5.stdout.isTTY ? process5.stdout.columns : undefined,
        getErrHelpWidth: () => process5.stderr.isTTY ? process5.stderr.columns : undefined,
    ...
    _executeSubCommand(subcommand, args) {
      ...
      if (process5.platform !== "win32") {
        if (launchWithNode) {
          ...
          proc4 = childProcess.spawn(process5.argv[0], args, { stdio: "inherit" });
        } else {
        ...
        args.unshift(executableFile);
        args = incrementNodeInspectorPort(process5.execArgv).concat(args);
        proc4 = childProcess.spawn(process5.execPath, args, { stdio: "inherit" });
      }
      if (!proc4.killed) {
...
`);
  ...
  function useColor() {
    if (process5.env.NO_COLOR || process5.env.FORCE_COLOR === "0" || process5.env.FORCE_COLOR === "false")
      return false;
    if (process5.env.FORCE_COLOR || process5.env.CLICOLOR_FORCE !== undefined)
      return true;
    return;
  }
...
`));
    searchResponse.objects.forEach(({ package: pkg }, index) => {
      this.logger.message(source_default.green(`${index + 1}. ${pkg.name}`));
...
class CodxCli {
  ...
  static run(process5) {
    ...
    if (process5.argv.length <= 2) {
      program2.outputHelp({ error: false });
      return;
    }
    program2.parse(process5.argv);
  }
